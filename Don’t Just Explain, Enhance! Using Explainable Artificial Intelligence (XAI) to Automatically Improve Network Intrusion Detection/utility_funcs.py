...
Utility functions for experiments 1 and 4
...
def VAE_anomaly_detection(model, train_data, test_data, ground_truth_locs, plt_title=None):    
    """ main function that applies variational autoencoder to the train and test sets and calculates performance of detector """
    
    # To find anomalies, first compute Reconstruction Error on Test data
    encoded_train, _, _ = model.encoder.predict(train_data)
    decoded_train = model.decoder.predict(encoded_train)
    #decoded_train = model.vae.predict(train_data)
    ...
    # Now compute the RE across the test points
    encoded_test, _, _, = model.encoder.predict(test_data)
    decoded_test = model.decoder.predict(encoded_test)
    #decoded_test = model.vae.predict(test_data)

...
data_loc = 'D:/Datasets/NSL_KDD/'               # location of dataset
...
results_AE_SHAP['shap_train'] = explainer.shap_values(data_kdd['X_train'])  # takes 7 min 24 secs first pass
results_AE_SHAP['shap_test'] = explainer.shap_values(data_kdd['X_test'])    # takes 1 min 19 secs


# scale data using Sklearn minmax scaler
shap_train_benign = results_AE_SHAP['shap_train'][ np.where( data_kdd['Y_train'] == "normal" )[0]]           # 67,343 samples isolate the explanations of training data which explain just benign samples
results_AE_SHAP['scaler'] = MinMaxScaler(feature_range=(-1,1))                                     
results_AE_SHAP['shap_train_norm_scaled'] = results_AE_SHAP['scaler'].fit_transform(shap_train_benign)       # scale the training set data
...
results_AE_SHAP['x_data'], results_AE_SHAP['val_data'] = train_test_split(results_AE_SHAP['shap_train_norm_scaled'], test_size=0.2, random_state=10)
...
results_AE_SHAP['parameters'] = {
                                    ...
                                    'dense_1_units':[800],                                   'dense_1_activation':['relu'],  # 32,64,128,256
                                    'dense_2_units':[80],                                 'dense_2_activation':['relu'],  # 32,48,64
                                    'dense_3_units':[16],             'dense_3_activation':['relu'], # (4,21,1) ->
                                    # decoder params to search across
                                    'dense_4_units':[448],                                        'dense_4_activation':['relu'],
                                    'dense_5_units':[2128],                                    'dense_5_activation':['relu'],  # 64,128,256
...
results_AE_SHAP['performance_new_attacks'], results_AE_SHAP['new_attack_pred_locs'], results_AE_SHAP['AE_threshold'] = utf.AE_anomaly_detection(autoencoder_shap, results_AE_SHAP['shap_train_norm_scaled'], results_AE_SHAP['shap_test_scaled'], data_kdd['new_attack_locs'], plt_title='Autoencoder trained on SHAP values')
...
y_prob[results_AE_SHAP['all_attack_pred_locs']] = 1.0
results_AE_SHAP['performance_overall']  = utf.compute_performance_stats(data_kdd['Y_test_bin'], results_AE_SHAP['y_pred_all'], y_prob)
